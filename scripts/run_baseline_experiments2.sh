...
# Run experiment on MiniGrid-DoorKey-8x8-v0
for episode in $(seq 1 $EPISODES); do
    episode_padded=$(printf "%03d" $episode)
    echo "Running experiments on MiniGrid-DoorKey-8x8-v0 - Episode $episode_padded..."
    episode_dir="${EXPERIMENT_DIR}/MiniGrid-DoorKey-8x8-v0/episode_${episode_padded}"
    mkdir -p "$episode_dir"
